...
export function activate(context: ExtensionContext) {
  ...
  let openSettingsPage = vscode.commands.registerCommand('luapanda.openSettingsPage', function () {
    ...
    try {
      ...
      DebugLogger.showTips(
        '解析 launch.json 文件失败, 请检查此文件配置项是否异常, 或手动修改 launch.json 中的项目来完成配置!',
        2
      );
    }
  });
  ...
  client = new LanguageClient('lua_analyzer', 'Lua Analyzer', serverOptions, clientOptions);
...
class LuaConfigurationProvider implements vscode.DebugConfigurationProvider {
  ...
  resolveDebugConfiguration(
    ...
    config.debugServer = (this._server.address() as any).port;
    return config;
  }
